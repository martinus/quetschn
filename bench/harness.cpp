...
run_result run_codec(corpus const& c, quetschn_codec const& codec, zsmalloc_model const& model, run_options const& opts) {
    ...
    auto workspace = std::vector<std::byte>(codec.workspace_size + cache_line);
    ...
    for (std::size_t i = 0; i < c.size(); ++i) {
        ...
        auto compress_once = [&]() -> unsigned int {
            ...
            if (codec.compress(src.data(), static_cast<unsigned int>(page_size), dst, &len, ws) != 0) {
        ...
        if (codec.decompress(dst, r.comp_len, out, &out_len) != 0) {
        ...
        if (opts.measure_time) {
            ...
            auto read_once = [&] {
                ...
                (void)codec.decompress(dst, r.comp_len, out, &len);
            };
