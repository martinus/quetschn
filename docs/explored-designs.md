...
## seqlz-opt: a literal table per page, less memory than `zstd` on both dumps

`seqlz-opt` codes the literals of a page with a table of its own when that saves at least 16 bytes,
otherwise with one of the 8 fixed tables as before. After zram's recompression that is 1.4% and 4.1%
less memory than before, and 3.8% and 1.8% less than `zstd` 3. The price is the reads: 17% and 34% of
the pages get their own table, the mean decode goes up by 10% and 13%, most of it for building the
table. In the kernel p99 of a cold read goes from 4809 to 5849 ns and from 5180 to
6020 ns, `lz4` has 4870 and 5080, `zstd` 9360 and 9630. Only `seqlz-opt` writes such pages:
`seqlz-fast-lit` decodes in 8193 and 7752 cycles per page, 8203 and 7740 before.
...
Tests: pages of 24 bytes as likely as each other all get their own table and come back, under 5 bits
