...
static int setup_opt(struct quetschn_params* p) {
    return setup(p, &seqlz_default_opt);
}

static void opt_destroy(struct quetschn_stream* s) {
    struct opt_ctx* c = s->context;

    if (!c)
        return;
    quetschn_free(c->work, &s->allocated);
    quetschn_free(c, &s->allocated);
    s->context = NULL;
}

static int opt_create(struct quetschn_params* p, struct quetschn_stream* s) {
    struct opt_ctx* c = quetschn_zalloc(sizeof(*c), &s->allocated);

    (void)p;
    if (!c)
        return -1;
    s->context = c;
    c->work = quetschn_zalloc(seqlz_opt_work_size(), &s->allocated);
    if (!c->work) {
        opt_destroy(s);
        return -1;
    }
...
static unsigned int page_lit_set(const unsigned char* src) {
...
static int opt_compress(struct quetschn_params* p,
                        struct quetschn_stream* s,
                        const void* src,
                        unsigned int src_len,
                        void* dst,
    ...
    if (src_len != SEQLZ_PAGE)
        return -1;
    len = seqlz_compress_opt(p->drv_data, src, dst, *dst_len, c->scratch, c->work, page_lit_set(src));
    if (!len)
        return -1;
    *dst_len = len;
    return 0;
...
static int opt_decompress(struct quetschn_params* p,
                          struct quetschn_stream* s,
                          const void* src,
                          unsigned int src_len,
                          void* dst,
...
const struct quetschn_codec quetschn_codec_seqlz_opt = {
    "seqlz-opt",
    setup_opt,
    release,
    opt_create,
